...
# Requires:
#   - dpm  >= 1.0.20  (oci:// component URIs; https://get.digitalasset.com/install/install.sh, then `dpm install 3.4.11`)
...
command -v dpm  >/dev/null 2>&1 || { echo "error: 'dpm' not found on PATH — install from https://get.digitalasset.com/install/install.sh (need >= 1.0.20) then 'dpm install 3.4.11'" >&2; exit 1; }
...
if [[ -z "${DPM_VERSION}" ]]; then
  ...
  echo "error: 'dpm' ${DPM_VERSION} is too old — install from https://get.digitalasset.com/install/install.sh (need >= 1.0.20) then 'dpm install 3.4.11'" >&2
  exit 1
fi
